 ...
 * @private 
 */ 
Events._domReadyTimer = null;
Events._domReadyCallback = [];   //zasobnik s objekty a jejich metodami, ktere chci zavolat po nastoleni udalosti
Events._domReadyAlreadyRun = false;/*ondomready je odchytavano specificky pro ruzne browsery a na konci je window.onload, tak aby se nespustilo 2x*/
Events._windowLoadListenerId = false; /*v nekterych prohlizecich pouzivame listener, pro jeho odveseni sem schovavam jeho id*/
 ...
 */ 
Events.onDomReady = function(obj, func) {
	Events._domReadyCallback[Events._domReadyCallback.length] = {obj: obj, func: func}
	Events._onDomReady();
}
 ...
 * @private
 */ 
Events._onDomReady = function() {
	var agent = navigator.userAgent,
		doc = document,
		domReady = Events._domReady,
		domReadyCallback = function(){domReady();};

	if((/Safari/i.test(agent)) || (/WebKit|Khtml/i.test(agent))){ //safari, konqueror
		Events._domReadyTimer=setInterval(function(){
			if(/loaded|complete/.test(doc.readyState)){
			    clearInterval(Events._domReadyTimer);
			    domReady(); // zavolani cilove metody
			}}, 10);
	} else if (doc.all && !win.opera){ //IE
		//v IE
		//nejsme v ramu
		if (win.parent == win) {
			try {
				// Diego Perini trik bez document.write, vice viz http://javascript.nwbox.com/IEContentLoaded/
				doc.documentElement.doScroll("left"); //test moznosti scrolovat, scrolovani jde dle msdn az po content load
			} catch( error ) {
			...
			Events._windowLoadListenerId = Events.addListener(win, 'load', win, domReadyCallback);
		}
	} else 	if (doc.addEventListener) { //FF, opera
		//Events._domReadyAlreadyRun = true;
  		doc.addEventListener("DOMContentLoaded", domReady, false); //FF, Opera ma specifickou udalost
  	} else {
...
Events._domReady = function() {
	...
	if (!Events._domReadyAlreadyRun) {
		...
		if (Events._windowLoadListenerId) {
			Events.removeListener(Events._windowLoadListenerId);
			Events._windowLoadListenerId = false;
		}
		
		//vlastni volani metody objektu
		for(i=0; i < Events._domReadyCallback.length; i++) {
			callback =  Events._domReadyCallback[i];
 ...
 */
Events.addListener = function(elm, type, obj, func, capture) {
 ...
 */    
Events._addListener = function(elm, type, action, capture){
	if (document.addEventListener) {
 ...
 */    
Events._getMethod = function(obj, func, elm, id) {
 ...
 */    
Events.removeListener = function(id) {
 ...
 */    
Events._removeListener = function(elm, type, action, capture) {
 ...
 */  
Events.removeListeners = function(array) {
	while(array.length) {
 ...
 */   
Events.removeAllListeners = function() {
...
Events.getInfo = function() {
	...
	for (id in Events._eventFolder) {
		o = Events._eventFolder[id];
		elm = o.elm;

		index = nodes.indexOf(elm);
		if (index == -1) {
	...
	for (i=0; i<nodes.length; i++) {
		...
		for (j=0; j<events[i].length; j++) {
			...
			listeners.push({
				'sType': o.type,
				'bRemoved': false,
				'sFunction':  (obj != win && obj.constructor ? '['+obj.constructor.NAME+']' : '') +
 ...
 */
ISignals.prototype.setInterface = function(interfaceName) {
 ...
 */
ISignals.prototype.addListener = function(type, handleFunction, sender) {
 ...
 */
ISignals.prototype.removeListener = function(id) {
 ...
 */  
ISignals.prototype.removeListeners = function(array) {
...
(function(){
	var AbstractDecorator =
		...
		AbstractDecoratorProto = AbstractDecorator.prototype;
 ...
 */
AbstractDecoratorProto.decorate = function(instance) {
 ...
 */
AbstractDecoratorProto._$super = function() {
	var caller = arguments.callee.caller;
	if (!caller) { throw new Error("Function.prototype.caller not supported"); }

	var decorators = this.__decorators || [],
		obj = null, /* objekt, jehoz metodu chceme volat */
		name = null, /* nazev metody */
		i = decorators.length,
		d,
		p,
		owner,
	...
	while (i--) { /* projdu vsechny naaplikovane dekoratory */
		...
		for (p in d) { /* hledame objekt s touto metodou a jeji nazev */
